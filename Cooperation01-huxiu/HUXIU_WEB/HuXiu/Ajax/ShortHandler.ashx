<%@ WebHandler Language="C#" Class="ShortHandler" %>
...
public class ShortHandler : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        else
        {
            ...
            {
                var select =( from it in db.News orderby it.NewsDate descending where it.IsDel==false select it).Take(size * index).Skip(size * (index - 1));
                if (index > 0)
                {

                    List<News> person = select.ToList();
                    DateHelper dt = new DateHelper();
                    foreach(var item in select)
                    {
                        item.NewsDate = dt.DateStringFromNow(Convert.ToDateTime(item.NewsDate));
                    }
