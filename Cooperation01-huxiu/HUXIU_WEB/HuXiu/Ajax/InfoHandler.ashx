...
public class InfoHandler :IHttpHandler,IRequiresSessionState {
    public void ProcessRequest(HttpContext context)
    {
        ...
        if(HttpContext.Current.Session["id"] != null)
            ...
            pageId = HttpContext.Current.Session["id"].ToString();

        else
            HttpContext.Current.Response.Redirect("~/index.aspx");

        if (string.IsNullOrEmpty(pageSize) || string.IsNullOrEmpty(pageIndex) || string.IsNullOrEmpty(pageId))
        {
        ...
        else
        {
            ...
            {
                var ds = (from it in db.PassageInformation where it.IsDel == false && it.PassageCategory == id orderby it.PublishDate descending select it).Take(size * index).Skip(size * (index - 1));
